...
```{r, message=FALSE}
setwd("~")
first <- read.csv("/Users/rsigalov/Documents/7Semester/R\ Econometrics\ Handbook/EURUSD_first.csv", sep = ";")
last <- read.csv("/Users/rsigalov/Documents/7Semester/R\ Econometrics\ Handbook/EURUSD_last.csv", sep = ";")
colnames(first)
```
I will do some manipulations with these files using `sql` language and show which columns do we have:
```{r, message=FALSE}
library("sqldf")
stats <- sqldf("
select
  f.Scenario_Group as scenario_group,
  f.Scenario as scenario,
  l.Spot - f.Spot as spot_chg,
  l.Position_Value + l.Acc_Trading_Gain - f.Option_Value as position_value_chg,
  l.Trades as trades,
  l.Option_Value - f.Option_Value as option_chg
from last as l 
  left join first as f on f.Scenario = l.Scenario and f.Scenario_Group = l.Scenario_Group
               ")
colnames(stats)
```
...
```{r}
price_data <- read.csv("/Users/rsigalov/Documents/7Semester/R\ Econometrics\ Handbook/price_dynamics.csv", sep = ",")
colnames(price_data)
